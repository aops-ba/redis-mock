...
function getSync (mockInstance, key) {
  var value = null;
  var err = null;
...
exports.get = function (mockInstance, key, callback) {
  ...
  var getResult = getSync(mockInstance, key);

  mockInstance._callCallback(callback, getResult[0], getResult[1]);
}
...
exports.getset = function (mockInstance, key, value, callback) {
  var getResult = getSync(mockInstance, key);
  var getErr = getResult[0];
  var oldValue = getResult[1];
  if (getErr) {
    return mockInstance._callCallback(callback, getErr, null);
  }

  mockInstance.storage[key] = createItem(value);

  mockInstance._callCallback(callback, getErr, oldValue);
};
